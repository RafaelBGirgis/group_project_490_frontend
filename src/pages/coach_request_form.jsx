...
function normalizeExistingAvailability(existingCoachProfile) {
  const next = { ...EMPTY_AVAILABILITY };
  const availabilities =
    existingCoachProfile?.coach_account?.availabilities ||
    existingCoachProfile?.coach_account?.availability ||
    existingCoachProfile?.availabilities ||
    existingCoachProfile?.availability ||
    [];

  (Array.isArray(availabilities) ? availabilities : []).forEach((slot) => {
    const weekday = String(slot?.weekday || "").trim().toLowerCase();
    const shortDay =
      weekday === "monday" ? "Mon" :
      weekday === "tuesday" ? "Tue" :
    ...
    const rawTime = String(slot?.start_time || "").trim().toUpperCase();
    ...
    if (AVAILABILITY_TIMES.includes(normalizedTime) && !next[shortDay].includes(normalizedTime)) {
      next[shortDay] = [...next[shortDay], normalizedTime];
    }
  });
    ...
    const loadNameFromSession = async () => {
      try {
        ...
        const existingSpecialties = String(existingCoachProfile?.coach_account?.specialties || "")
          ...
          .map((item) => item.trim())
          .filter(Boolean);
        const existingAvailability = normalizeExistingAvailability(existingCoachProfile);
